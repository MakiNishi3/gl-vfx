// Fork of "Toni Tunnel" by Kali. https://shadertoy.com/view/ttVyW3
// 2022-06-04 09:57:40
...
vec3 fractal(vec3 p)
{
    ...
    {
        p.xy *= rot(z);
        p = abs(p * (1.5+sin(z*1.3)*.2)) - 2.;
        m = min(m, abs(p.y) + .5 * abs(.5 - fract(p.x * 2.25 + iTime + float(i) * 1.1)));
    }
    m = exp(-8. * m) * 2.;
    return vec3(p.xz * 2., m) * m + .25;
}
