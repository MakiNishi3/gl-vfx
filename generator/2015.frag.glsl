...
#define beat1 (texture(iChannel0, vec2(0.1)).r * 2.)
#define beat2 (texture(iChannel0, vec2(0.8)).r * 2.)
...
const vec3 c1 = vec3(0.1, 0.3, 0.9) * 1.;
const vec3 c2 = vec3(0.4, 0.1, 0.2) * 1.;
const vec3 c3 = vec3(0.3, 0.08, 0.05) * 0.25;
const vec3 bg = vec3(0.2, 0.5, 0.925);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
  ...
  ta  = vec3(0, 0, iChannelTime[0] * 2.75);
  ro = ta - vec3(0, 0, 2.5);

  l1 = ro + vec3(0, sin(iChannelTime[0] * 5.) * 0.5, 11.25 + cos(iChannelTime[0] * 0.97) * 9.);
  l2 = ta + vec3(0, 0, 5.25 + sin(iChannelTime[0] * 0.85) * 7.);
