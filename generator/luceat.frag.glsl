...
{
    //amplitude envelope
    audioEnvelope = (texture(iChannel2, vec2(iChannelTime[1],0.0))).x;
   	int c =0;
  	for(float k = 0.0; k<0.02; k+=0.001)
    {
    	c++;
    	float val = abs((texture(iChannel0, vec2(iChannelTime[1]+k,0.0))).x);
	...
	vec3 lightPos = vec3(0.8*sin(iTime*3.2/4.0), 0.8*sin(iTime*1.2/4.0), 0.0);
