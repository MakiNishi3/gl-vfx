// adaptation of "wave matrix product" by pixelbeast aka Iain Melvin 2014
...
float get_max(){
  ...
  for (int j=0;j<800;j++){
    jf = jf+0.00125;
    ja = texture( iChannel0, vec2(jf,.75)).x;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec4 t2 = texture(iChannel0, vec2(uv[1],0.761)+jmax )-0.5;
   	float y = t1[0]*t2[0]*a;
    vec4 c = vec4( mod(y*23.,1.), mod(y*7.5,1.),mod(y*12.,1.),1.0);
    c *= text;
	fragColor = c;
}
